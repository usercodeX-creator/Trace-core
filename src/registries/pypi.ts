...
export async function exists(packageName: string): Promise<boolean> {
  ...
  try {
    ...
    throw new Error(`PyPI returned unexpected status ${response.status} for package "${packageName}"`);
  } catch (error) {
    if (error instanceof DOMException && error.name === "AbortError") {
      throw new Error(`PyPI request timed out after ${TIMEOUT_MS}ms for package "${packageName}"`);
    }
    throw error;
  } finally {
